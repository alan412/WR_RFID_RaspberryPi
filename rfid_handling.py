...
class members(object):
    ...
    def readFile(self, filename):
        ...
        with open(filename, 'r') as csvfile:
            ...
            headers = next(csvreader, None) # returns the headers or None if it is empty
     	    for row in csvreader:
                if row[0]:
     		       self.students[row[0]] = {'rfid': row[0], 'name': row[1] + ' ' + row[2], 'isHere': False}
...
class RFIDHandling(object):
    def __init__(self, fileBase = "/home/pi/rfid/", location = "Hopewell"):
    ...
    def blockingRead(self):
        ...
        self.members.writePresent(self.fileBase + "/web/users.txt")
